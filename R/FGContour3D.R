...
FGContour3D<-function(data,trainModel,contCov,contCovName=NULL,nCovEval=30,otherCov=NULL){
  if(is.null(otherCov)){
  ...
  pred<-NULL
  pred$surv <- predictRisk(object = trainModel, newdata =newData,times = ctime)
  if(is.null(contCovName)){
    contCovName<-contCov
  }
  s<-contour3D(ctime,newData[,contCov],pred,contCovName,FALSE,TRUE)
  s
