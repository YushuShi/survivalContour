...
paraContour3D<-function(data,trainModel,contCov,contCovName=NULL,nCovEval=30,otherCov=NULL,CI3D=FALSE){
  if(is.null(otherCov)){
  ...
  z<-NULL
  z$surv<-matrix(predictPlot$.pred_survival,ncol=length(time),byrow = TRUE)
  z$upper<-matrix(predictPlot$.pred_upper,ncol=length(time),byrow = TRUE)
  z$lower<-matrix(predictPlot$.pred_lower,ncol=length(time),byrow = TRUE)
  ...
  s<-contour3D(time,newData[,contCov],z,contCovName,CI3D)
  s
}
